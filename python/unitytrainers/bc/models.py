...
class BehavioralCloningModel(LearningModel):
    def __init__(self, brain, h_size=128, lr=1e-4, n_layers=2, m_size=128,
        ...
        hidden_reg = tf.compat.v1.layers.dropout(hidden, self.dropout_rate)
        ...
        self.policy = tf.compat.v1.layers.dense(hidden_reg, self.a_size, activation=None, use_bias=False, name='pre_action',
                                      kernel_initializer=c_layers.variance_scaling_initializer(factor=0.01))

        if brain.vector_action_space_type == "discrete":
            self.action_probs = tf.nn.softmax(self.policy)
            self.sample_action_float = tf.random.categorical(logits=self.policy, num_samples=1)
            ...
            self.true_action = tf.compat.v1.placeholder(shape=[None], dtype=tf.int32, name="teacher_action")
            self.action_oh = tf.one_hot(self.true_action, self.a_size)
            self.loss = tf.reduce_sum(input_tensor=-tf.math.log(self.action_probs + 1e-10) * self.action_oh)
            self.action_percent = tf.reduce_mean(input_tensor=tf.cast(
                tf.equal(tf.cast(tf.argmax(input=self.action_probs, axis=1), tf.int32), self.sample_action), tf.float32))
        else:
            ...
            self.true_action = tf.compat.v1.placeholder(shape=[None, self.a_size], dtype=tf.float32, name="teacher_action")
            self.clipped_true_action = tf.clip_by_value(self.true_action, -1, 1)
            self.loss = tf.reduce_sum(input_tensor=tf.math.squared_difference(self.clipped_true_action, self.sample_action))
