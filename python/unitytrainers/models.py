...
class LearningModel(object):
    def __init__(self, m_size, normalize, use_recurrent, brain):
        ...
        self.sequence_length = tf.compat.v1.placeholder(shape=None, dtype=tf.int32, name='sequence_length')
    ...
    def create_global_steps():
        ...
        increment_step = tf.compat.v1.assign(global_step, tf.add(global_step, 1))
    ...
    def create_visual_input(camera_parameters, name):
        ...
        visual_in = tf.compat.v1.placeholder(shape=[None, o_size_h, o_size_w, c_channels], dtype=tf.float32, name=name)
        return visual_in
    ...
    def create_vector_input(self, name='vector_observation'):
        ...
        if self.brain.vector_observation_space_type == "continuous":
            self.vector_in = tf.compat.v1.placeholder(shape=[None, self.o_size], dtype=tf.float32, name=name)
            if self.normalize:
                self.running_mean = tf.compat.v1.get_variable("running_mean", [self.o_size], trainable=False, dtype=tf.float32,
                                                    initializer=tf.compat.v1.zeros_initializer())
                self.running_variance = tf.compat.v1.get_variable("running_variance", [self.o_size], trainable=False,
            ...
            self.vector_in = tf.compat.v1.placeholder(shape=[None, 1], dtype=tf.int32, name='vector_observation')
            return self.vector_in

    def create_normalizer_update(self, vector_input):
        mean_current_observation = tf.reduce_mean(input_tensor=vector_input, axis=0)
        ...
        update_mean = tf.compat.v1.assign(self.running_mean, new_mean)
        update_variance = tf.compat.v1.assign(self.running_variance, new_variance)
    ...
    def create_continuous_observation_encoder(observation_input, h_size, activation, num_layers, scope, reuse):
        ...
        with tf.compat.v1.variable_scope(scope):
            ...
            for i in range(num_layers):
                hidden = tf.compat.v1.layers.dense(hidden, h_size, activation=activation, reuse=reuse, name="hidden_{}".format(i),
    ...
    def create_visual_observation_encoder(self, image_input, h_size, activation, num_layers, scope, reuse):
        ...
        """
        with tf.compat.v1.variable_scope(scope):
            conv1 = tf.compat.v1.layers.conv2d(image_input, 16, kernel_size=[8, 8], strides=[4, 4],
                                     activation=tf.nn.elu, reuse=reuse, name="conv_1")
            conv2 = tf.compat.v1.layers.conv2d(conv1, 32, kernel_size=[4, 4], strides=[2, 2],
    ...
    def create_discrete_observation_encoder(observation_input, s_size, h_size, activation,
        ...
        with tf.compat.v1.variable_scope(scope):
            ...
            hidden = state_onehot
            for i in range(num_layers):
                hidden = tf.compat.v1.layers.dense(hidden, h_size, use_bias=False, activation=activation,
    ...
    def create_recurrent_encoder(input_state, memory_in, sequence_length, name='lstm'):
        ...
        with tf.compat.v1.variable_scope(name):
            rnn_cell = tf.compat.v1.nn.rnn_cell.BasicLSTMCell(_half_point)
            lstm_vector_in = tf.compat.v1.nn.rnn_cell.LSTMStateTuple(memory_in[:, :_half_point], memory_in[:, _half_point:])
            recurrent_output, lstm_state_out = tf.compat.v1.nn.dynamic_rnn(rnn_cell, lstm_input_state,
    ...
    def create_dc_actor_critic(self, h_size, num_layers):
        ...
        if self.use_recurrent:
            ...
            self.prev_action = tf.compat.v1.placeholder(shape=[None], dtype=tf.int32, name='prev_action')
            prev_action_oh = tf.one_hot(self.prev_action, self.a_size)
            hidden = tf.concat([hidden, prev_action_oh], axis=1)

            self.memory_in = tf.compat.v1.placeholder(shape=[None, self.m_size], dtype=tf.float32, name='recurrent_in')
            hidden, memory_out = self.create_recurrent_encoder(hidden, self.memory_in, self.sequence_length)
            self.memory_out = tf.identity(memory_out, name='recurrent_out')

        self.policy = tf.compat.v1.layers.dense(hidden, self.a_size, activation=None, use_bias=False,
                                      kernel_initializer=c_layers.variance_scaling_initializer(factor=0.01))

        self.all_probs = tf.nn.softmax(self.policy, name="action_probs")
        output = tf.random.categorical(logits=self.policy, num_samples=1)
        self.output = tf.identity(output, name="action")

        value = tf.compat.v1.layers.dense(hidden, 1, activation=None)
        self.value = tf.identity(value, name="value_estimate")
        self.entropy = -tf.reduce_sum(input_tensor=self.all_probs * tf.math.log(self.all_probs + 1e-10), axis=1)
        ...
        self.all_old_probs = tf.compat.v1.placeholder(shape=[None, self.a_size], dtype=tf.float32, name='old_probabilities')

        # We reshape these tensors to [batch x 1] in order to be of the same rank as continuous control probabilities.
        self.probs = tf.expand_dims(tf.reduce_sum(input_tensor=self.all_probs * self.selected_actions, axis=1), 1)
        self.old_probs = tf.expand_dims(tf.reduce_sum(input_tensor=self.all_old_probs * self.selected_actions, axis=1), 1)
    ...
    def create_cc_actor_critic(self, h_size, num_layers):
        ...
        mu = tf.compat.v1.layers.dense(hidden_policy, self.a_size, activation=None,
                             kernel_initializer=c_layers.variance_scaling_initializer(factor=0.01))

        log_sigma_sq = tf.compat.v1.get_variable("log_sigma_squared", [self.a_size], dtype=tf.float32,
                                       initializer=tf.compat.v1.zeros_initializer())

        sigma_sq = tf.exp(log_sigma_sq)

        epsilon = tf.random.normal(tf.shape(input=mu), dtype=tf.float32)
        ...
        self.entropy = tf.reduce_mean(input_tensor=0.5 * tf.math.log(2 * np.pi * np.e * sigma_sq))

        value = tf.compat.v1.layers.dense(hidden_value, 1, activation=None)
        self.value = tf.identity(value, name="value_estimate")

        self.all_old_probs = tf.compat.v1.placeholder(shape=[None, self.a_size], dtype=tf.float32,
